...
from mmocr_.structures import TextDetDataSample
from mmocr_.utils.typing_utils import ArrayLike
...
class FCEModuleLoss(TextSnakeModuleLoss):
    ...
    def _cal_fourier_signature(self, polygon: ArrayLike,
        ...
        """
        resampled_polygon = self._resample_polygon(polygon)             # 采样400个点
        resampled_polygon = self._normalize_polygon(resampled_polygon)  # 使得起始点在最右边

        fourier_coeff = self._poly2fourier(resampled_polygon, fourier_degree)   # 边框点输入，得到傅里叶系数
        fourier_coeff = self._clockwise(fourier_coeff, fourier_degree)          # 使得傅里叶系数从大到小排列
