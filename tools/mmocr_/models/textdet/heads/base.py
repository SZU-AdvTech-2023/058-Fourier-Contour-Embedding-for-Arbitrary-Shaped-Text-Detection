...
from mmocr_.utils.typing_utils import DetSampleList
...
class BaseTextDetHead(BaseModule):
    ...
    # (1) head网络前向传播得到特征图 (2)根据特征图调用module_loss方法得到loss
    .. code:: text
    ...
    post-processing.
    # (1) head网络前向传播得到特征图 (2)根据特征图调用postprocessor方法得到检测结果，包括后处理
    .. code:: text
    ...
    # 同时返回loss和检测结果，按顺序调用forward、module_loss和postprocessor方法
    .. code:: text
